...
class SimpleSpreadMPE(SimpleMPE):
    ...
    def _get_high_level_action(self, pos, land_pos, radius=0.25):

        # landmark positions in agents reference frame
        ...
        def no_lm(_):
            return jnp.zeros(self.num_landmarks)

        def yes_lm(_):
            return jnp.zeros(self.num_landmarks).at[min_idx].set(1)
