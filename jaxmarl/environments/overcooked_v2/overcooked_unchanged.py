...
lookup_reward_shaped = jnp.array([
    [0,  0],  
...
class Overcooked_v2(MultiAgentEnv):
    ...
    def get_obs(self, state: State) -> Dict[str, chex.Array]:
        ...
        for (num_onion, num_tomato) in [(1, 0), (2, 0), (3, 0), (0, 1), (0, 2), (0, 3), (1, 1), (2, 1), (1, 2)]:
            ...
            # jax.debug.print("ready_status: {}",  ready_status)
            # jax.debug.print("in_pot_status: {}",  in_pot_status)
            # jax.debug.print("pot_status: {}",  pot_status)
            # pot_status_bigger = (pot_status >= ready_status)
            # jax.debug.print("pot_status_bigger: {}",  pot_status_bigger)
            # jax.debug.print("pot_status dtype: {}", pot_status.dtype)
            # jax.debug.print("ready_status dtype: {}", jax.numpy.array(ready_status).dtype)
            # jax.debug.print("in_pot_status dtype: {}", jax.numpy.array(in_pot_status).dtype)
