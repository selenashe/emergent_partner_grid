...
DISH_TYPE_TO_REWARD = {
    ...
    4: 0, # 2 tomato
    5: 0, # 3 tomato
    6: 0, # 1 onion 1 tomato
    7: 0, # 1 onion 2 tomato
    8: 0, # 2 onion 1 tomato
}
...
OBJECT_INDEX_TO_VEC = jnp.array([
	...
	jnp.array([OBJECT_TO_INDEX['tomato'], COLOR_TO_INDEX["red"], 0], dtype=jnp.uint8),
	jnp.array([OBJECT_TO_INDEX['tomato_pile'], COLOR_TO_INDEX["red"], 0], dtype=jnp.uint8),
	...
	jnp.array([OBJECT_TO_INDEX['dish_0'], COLOR_TO_INDEX["white"], 0], dtype=jnp.uint8),
	jnp.array([OBJECT_TO_INDEX['dish_1'], COLOR_TO_INDEX["white"], 0], dtype=jnp.uint8),
	jnp.array([OBJECT_TO_INDEX['dish_2'], COLOR_TO_INDEX["white"], 0], dtype=jnp.uint8),
	jnp.array([OBJECT_TO_INDEX['dish_3'], COLOR_TO_INDEX["white"], 0], dtype=jnp.uint8),
	jnp.array([OBJECT_TO_INDEX['dish_4'], COLOR_TO_INDEX["white"], 0], dtype=jnp.uint8),
	jnp.array([OBJECT_TO_INDEX['dish_5'], COLOR_TO_INDEX["white"], 0], dtype=jnp.uint8),
	jnp.array([OBJECT_TO_INDEX['dish_6'], COLOR_TO_INDEX["white"], 0], dtype=jnp.uint8),
	jnp.array([OBJECT_TO_INDEX['dish_7'], COLOR_TO_INDEX["white"], 0], dtype=jnp.uint8),
	jnp.array([OBJECT_TO_INDEX['dish_8'], COLOR_TO_INDEX["white"], 0], dtype=jnp.uint8),
...
def make_overcooked_map(
	...
	if onion_pile_pos.size > 0:
 	...
 	# Add tomatoes
	if tomato_pile_pos.size > 0:
		tomato_x = tomato_pile_pos[:,0]
		tomato_y = tomato_pile_pos[:,1]
		tomato_pile = jnp.array([OBJECT_TO_INDEX['tomato_pile'], COLOR_TO_INDEX["red"], 0], dtype=jnp.uint8)
		maze_map = maze_map.at[tomato_y, tomato_x, :].set(tomato_pile)
	...
	if len(tomato_pos) > 0:
		tomato_x = tomato_pos[:, 0]
		tomato_y = tomato_pos[:, 1]
		tomato = jnp.array([OBJECT_TO_INDEX['tomato'], COLOR_TO_INDEX["red"], 0], dtype=jnp.uint8)
		maze_map = maze_map.at[tomato_y, tomato_x, :].set(tomato)
	if len(plate_pos) > 0:
		plate_x = plate_pos[:, 0]
		plate_y = plate_pos[:, 1]
	...
	if len(dish_pos) > 0:
		...
		dish_y = dish_pos[:, 1]
		dish = jnp.stack(
			[jnp.array([OBJECT_TO_INDEX[f'dish_{type}'], COLOR_TO_INDEX["white"], 0]) for type in dish_type],
			dtype=jnp.uint8
		)
