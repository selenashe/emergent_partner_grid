...
class State:
    ...
    behaviour_mode_agent_0: int = 0
    behaviour_mode_agent_1: int = 0    
...
class Overcooked_v2(MultiAgentEnv):
    ...
    def __init__(
            ...
            max_steps: int = 400,
            rew_shaping_params_agent_0=jnp.array([3, 3, 3, 5, 0]),  # Default values
            rew_shaping_params_agent_1=jnp.array([1, 1, 1, 2, 0]),
            main_rew_is_shaped_rew=jnp.float32(0) # Default False
    ):
        ...
        self.obs_shape = (self.width, self.height, 28)
        ...
        self.rew_shaping_params_agent_0 = rew_shaping_params_agent_0
        self.rew_shaping_params_agent_1 = rew_shaping_params_agent_1
        self.main_rew_is_shaped_rew = main_rew_is_shaped_rew

        # === ADD THE FOLLOWING ===
        self.current_behaviour_mode_agent_0 = 0
        self.current_behaviour_mode_agent_1 = 0

        # self.behaviour_modes_agent_0 = jnp.array([
        #     [1, 1, 1, -10, -10],
        #     [-10, -10, -10, 1, 1]
        # ])

        # self.behaviour_modes_agent_1 = jnp.array([
        #     [1, 1, 1, -10, -10],
        #     [-10, -10, -10, 1, 1]
        # ])
        self.behaviour_modes_agent_0 = jnp.array([
    ...
    # def set_behaviour_mode_agent_0(self, mode):
    #     self.current_behaviour_mode_agent_0 = mode
    #     self.rew_shaping_params_agent_0 = self.behaviour_modes_agent_0[mode]

    # def set_behaviour_mode_agent_1(self, mode):
    #     self.current_behaviour_mode_agent_1 = mode
    #     self.rew_shaping_params_agent_1 = self.behaviour_modes_agent_1[mode]


    def schedule_mode(self, t: int, agent_id: int) -> int:
        def random_flip(_):
            offset = 999_999 * agent_id
            key = jax.random.PRNGKey(t + offset)
            rand_val = jax.random.uniform(key)
            switch_probability = 0.05

            current_mode = jax.lax.select(
                agent_id == 0,
                self.current_behaviour_mode_agent_0,
                self.current_behaviour_mode_agent_1
            )

            return jax.lax.select(rand_val < switch_probability,
                                1 - current_mode,
                                current_mode)

        mode = random_flip(None)
        return mode


    # def schedule_mode(self, t: int, agent_id: int) -> int:
    #     """
    #     A purely functional phase-based + random schedule, JAX-friendly.

    #     Example phases:
    #     Phase 0: t < 2000 => mode = 0
    #     Phase 1: 2000 <= t < 4000 => mode = 1
    #     Phase 2: t >= 4000 => random flipping with 10% probability

    #     The agent_id must be an integer: 0 for agent_0, 1 for agent_1.
    #     This function uses jax.lax.cond (functional if) to avoid
    #     TracerBoolConversionError. Also uses an offset in the PRNG key
    #     to differentiate random flips between agent_0 and agent_1.
    #     """
    ...
    #     def random_flip(_):
    #         # Distinguish the random seed for each agent
    #         offset = 999_999 * agent_id
    #         key = jax.random.PRNGKey(t + offset)
    #         rand_val = jax.random.uniform(key)
    #         switch_probability = 0.05

    #         # Current mode depends on which agent we're scheduling
    #         current_mode = jax.lax.select(
    #             agent_id == 0,
    #             self.current_behaviour_mode_agent_0,
    #             self.current_behaviour_mode_agent_1
    ...
    #         # Flip with probability switch_probability
    #         return jax.lax.select(rand_val < switch_probability,
    #                             1 - current_mode,
    #                             current_mode)
    ...
    def step_env(
        ...
        state, reward, shaped_rewards, high_level_dicts = self.step_agents(key, state, acts)
        ...
        high_level_actions = {"agent_0":  high_level_dicts[0], "agent_1": high_level_dicts[1]}

        # Shift the action history to the left and add the new actions
        new_action_history = jnp.roll(state.action_history, shift=-1, axis=1)
        new_action_history = new_action_history.at[:, -1].set(acts)
        state = state.replace(action_history=new_action_history)

        ########################################
        # new_mode_0 = self.schedule_mode(state.time, 0)
        # new_mode_1 = self.schedule_mode(state.time, 1)

        # state = state.replace(
        #     behaviour_mode_agent_0=new_mode_0,
        #     behaviour_mode_agent_1=new_mode_1
        # )

        # self.set_behaviour_mode_agent_0(new_mode_0)
        # self.set_behaviour_mode_agent_1(new_mode_1)
        ########################################
    ...
    def reset(
        ...
        # ! change for 1-onion soup
        valid_pot_statuses = jnp.array([STATUSES_INDEX['empty']] + 
                                    [STATUSES_INDEX['1_onion_0_tomato_ready'],
                                        STATUSES_INDEX['1_onion_0_tomato_in_pot']])
        pot_status = jax.random.choice(subkey, valid_pot_statuses, shape=(pot_idx.shape[0],))
        # ! end of change
        ...
        history_length = 1
        action_history = jnp.full((self.num_agents, history_length), Actions.stay, dtype=jnp.int32)
        ...
        state = State(
            ...
            behaviour_mode_agent_0=self.current_behaviour_mode_agent_0,
            behaviour_mode_agent_1=self.current_behaviour_mode_agent_1,            
        )
    ...
    def get_obs(self, state: State) -> Dict[str, chex.Array]:
        ...
        urgency_layer = jnp.ones(maze_map.shape, dtype=jnp.uint8) * 0
        ...
        alice_obs = alice_obs.at[10:26].set(jnp.stack(env_layers))
        ...
        bob_obs = bob_obs.at[10:26].set(jnp.stack(env_layers))

        # Add the agent ID channels to alice_obs and bob_obs
        agent_0_id = jnp.full((1, height, width), 0.0, dtype=jnp.uint8)  # Agent 0 ID
        agent_1_id = jnp.full((1, height, width), 1.0, dtype=jnp.uint8)  # Agent 1 ID

        alice_obs = alice_obs.at[26:].set(agent_0_id)
        bob_obs = bob_obs.at[26:].set(agent_1_id)

        #########################################
        alice_obs = alice_obs.at[:, :, 27].set(state.behaviour_mode_agent_0)
        bob_obs = bob_obs.at[:, :, 27].set(state.behaviour_mode_agent_1)
        #########################################
        ...
        ##################### Making everything ################
        # alice_mask = jnp.zeros_like(alice_obs)
        # alice_mask = alice_mask.at[:, :, 0].set(1)  # Preserve own position
        # alice_mask = alice_mask.at[:, :, 26].set(1) # Preserve agent ID
        # alice_obs = alice_obs * alice_mask

        # bob_mask = jnp.zeros_like(bob_obs)
        # bob_mask = bob_mask.at[:, :, 0].set(1)      # Preserve own position
        # bob_mask = bob_mask.at[:, :, 26].set(1)       # Preserve agent ID
        # bob_obs = bob_obs * bob_mask
    ...
    def step_agents(
        ...
        candidate_maze_map, alice_inv, alice_reward, alice_shaped_reward, alice_high_level_dict = self.process_interact(maze_map, state.wall_map, fwd_pos, state.agent_inv, 0, state.action_history, state.time, self.behaviour_modes_agent_0[state.behaviour_mode_agent_0])
        ...
        candidate_maze_map, bob_inv, bob_reward, bob_shaped_reward, bob_high_level_dict = self.process_interact(maze_map, state.wall_map, fwd_pos, state.agent_inv, 1, state.action_history, state.time, self.behaviour_modes_agent_1[state.behaviour_mode_agent_1])
        ...
        ##########################################
        # alice_high_level_dict = jax.lax.select(alice_interact, alice_high_level_dict, 0.)
        # bob_high_level_dict = jax.lax.select(bob_interact, bob_high_level_dict, 0.)

        def convert_to_jax_arrays(high_level_dict):
            return {k: jnp.array(v) if isinstance(v, np.ndarray) else v for k, v in high_level_dict.items()}

        # Apply conversion to Alice and Bob's high-level dictionaries
        alice_high_level_dict = convert_to_jax_arrays(alice_high_level_dict)
        bob_high_level_dict = convert_to_jax_arrays(bob_high_level_dict)

        def process_high_level_dict(high_level_dict, interact_flag):
            # Zero out all elements conditionally
            def zero_out(x):
                return jnp.where(interact_flag, x, 0)

            # Apply the zeroing transformation
            return tree_util.tree_map(zero_out, high_level_dict)

        # Process Alice and Bob's dictionaries
        alice_high_level_dict = process_high_level_dict(alice_high_level_dict, alice_interact)
        bob_high_level_dict = process_high_level_dict(bob_high_level_dict, bob_interact)

        # from jax import tree_util
        # def zero_out_dict(d):
        #     return tree_util.tree_map(lambda x: 0, d)

        # # Use `jax.lax.select` to conditionally apply the zeroing
        # alice_high_level_dict = jax.lax.select(
        #     alice_interact,
        #     alice_high_level_dict,
        #     zero_out_dict(alice_high_level_dict)
        # )

        # bob_high_level_dict = jax.lax.select(
        #     bob_interact,
        #     bob_high_level_dict,
        #     zero_out_dict(bob_high_level_dict)
        # )
        ##########################################
        ...
        return (
            ...
            (alice_high_level_dict, bob_high_level_dict)
        )
    ...
    def process_interact(
            ...
            player_idx: int,
            action_history_all: chex.Array,
            time: int,
            agent_preference: chex.Array):
        """Assume agent took interact actions. Result depends on what agent is facing and what it is holding."""

        # jax.debug.print("new_action_history: {}",  action_history)

        # CALCULATE HISTORY
        # jax.debug.print("action_history: {}",  action_history)

        action_history = action_history_all[player_idx]
        
        # action_value = 4
        # max_history_length = 100 
        # mask = jnp.arange(100) < time
        # count_4s = jnp.sum((action_history * mask) == action_value)
        # percentage_4s = count_4s / 100
        # target_percentage_4s = 0.3
        # tolerance = 0.05
        # deviation = jnp.abs(percentage_4s - target_percentage_4s)
        # reward_4s = -deviation
        # reward_4s = -deviation ** 2
        # reward_4s = reward_4s * (time > 100) * player_idx * 0

        # jax.debug.print("player_idx: {}",  player_idx)

        # action_history_to_time = action_history[:time-1]
        # action_history_to_time = lax.dynamic_slice(action_history, (0,), (time,))
        ...
        has_picked_up_plate_old = successful_pickup*(object_in_inv == OBJECT_TO_INDEX["plate"])
        ...
        reward = jnp.array(successful_delivery_numbers, dtype=float)
        reward_any_dish = jnp.array(successful_delivery, dtype=float)
        ...
        # AGENT 0 PREFERENCE: [OP, TP, PP, BP, PS]
        # AGENT 1 PREFERENCE: [OP, TP, PP, BP, PS]

        # agent_0_preference = [0, 10, 0, 0, 0]
        # agent_1_preference = [0, 10, 0, 0, 0]

        # [OP, TP, PP, BP, PS]
        # AGENT 0 = RED, AGENT 1 = BLUE

        # [OP, TP, PP, BP, PS]
        OP_1 = 1
        # agent_0_preference = [OP_1, 1, OP_1, 1, 20]
        # agent_0_preference = [20, 0, 0, -100, -100]
        # agent_0_preference = [-1000, 0, -10, 20, 20]
        # agent_0_preference = [6.33467845, -2.11155979, 6.33467845, -2.1115591 , -2.1115591 ]

        OP_2 = 1
        # agent_1_preference = [OP_2, 1, OP_2, 1, 20]

        agent_0_preference = self.rew_shaping_params_agent_0
        agent_1_preference = self.rew_shaping_params_agent_1


        # agent_0_preference = agent_preference
        # agent_1_preference = agent_preference


        # agent_1_preference = [20, 0, 0, -100, -100]
        # agent_1_preference = [-1000, 0, -10, 20, 20]

        # agent_1_preference = [1.42680225, 5.66705008, 1.42680225, 2.12007104,  2.12007025]

        # option1
        # agent_0_preference = [ 0.24520233, -0.44794746,  0.24520233, 0.24520228, -0.04245732]
        # agent_1_preference = [ 2.31075049, -5.54570341,  2.31075049,1.61743122,  1.61743855]

        # # option2
        # agent_0_preference = [-0.10136588,  0.3040976 , -0.10136588, -0.10136588, -0.10136588]
        # agent_1_preference = [ 2.31075049, -5.54570341, 2.31075049, 1.61743122,  1.61743855]
        # # option3
        # agent_0_preference = [ 2.91554871,  2.91554871, 2.91554871,-2.91559698, -2.91559632]
        # agent_1_preference = [-2.91559873, -2.91559873,-2.91559873,  2.91554806,  2.91554806]

        # agent_0_preference = [1,1,1,1,1]
        # agent_1_preference = [1,1,1,1,1]

        shaped_reward_0 = 0.
        shaped_reward_0 += (num_onions_in_pot < num_onions_in_recipe) * add_onion_to_pot * agent_0_preference[0]
        shaped_reward_0 += (num_tomatoes_in_pot < num_tomatoes_in_recipe) * add_tomato_to_pot * agent_0_preference[1]
        shaped_reward_0 += case_4 * 1 * (num_onions_in_pot == num_onions_in_recipe) * (num_tomatoes_in_pot == num_tomatoes_in_recipe) * agent_0_preference[2]
        shaped_reward_0 += case_3 * 1 * (num_onions_in_pot == num_onions_in_recipe) * (num_tomatoes_in_pot == num_tomatoes_in_recipe) * agent_0_preference[3]
        shaped_reward_0 -= case_4 * object_is_pot * (num_onions_in_pot != num_onions_in_recipe) * 1
        shaped_reward_0 -= case_4 * object_is_pot * (num_tomatoes_in_pot != num_tomatoes_in_recipe) * 1
        shaped_reward_0 += reward * agent_0_preference[4]

        shaped_reward_1 = 0.
        shaped_reward_1 += (num_onions_in_pot < num_onions_in_recipe) * add_onion_to_pot * agent_1_preference[0]
        shaped_reward_1 += (num_tomatoes_in_pot < num_tomatoes_in_recipe) * add_tomato_to_pot * agent_1_preference[1]
        shaped_reward_1 += case_4 * 1 * (num_onions_in_pot == num_onions_in_recipe) * (num_tomatoes_in_pot == num_tomatoes_in_recipe) * agent_1_preference[2]
        shaped_reward_1 += case_3 * 1 * (num_onions_in_pot == num_onions_in_recipe) * (num_tomatoes_in_pot == num_tomatoes_in_recipe) * agent_1_preference[3]
        shaped_reward_1 -= case_4 * object_is_pot * (num_onions_in_pot != num_onions_in_recipe) * 1
        shaped_reward_1 -= case_4 * object_is_pot * (num_tomatoes_in_pot != num_tomatoes_in_recipe) * 1
        shaped_reward_1 += reward * agent_1_preference[4]        
        ...
        # AGENT 0 = RED, AGENT 1 = BLUE
        # shaped_reward_1 = 0.
        # # PUTTING ONIONS IN POT
        # shaped_reward_1 += (num_onions_in_pot < num_onions_in_recipe) * add_onion_to_pot * preference_put_onions

        # # PUTTING TOMATOES IN POT
        # shaped_reward_1 += (num_tomatoes_in_pot < num_tomatoes_in_recipe) * add_tomato_to_pot * preference_put_tomatoes
        # # STARTING COOKING
        # shaped_reward_1 += case_4 * 1 * (num_onions_in_pot == num_onions_in_recipe) * (num_tomatoes_in_pot == num_tomatoes_in_recipe) * preference_start_cooking
        # # shaped_reward_1 -= object_is_pot * (num_onions_in_pot != num_onions_in_recipe) * case_4 * 1
        # # shaped_reward_1 -= object_is_pot * (num_tomatoes_in_pot != num_tomatoes_in_recipe) * case_4 * 1
        # # PICKING UP SOUP
        # shaped_reward_1 += case_3 * 1 * (num_onions_in_pot == num_onions_in_recipe) * (num_tomatoes_in_pot == num_tomatoes_in_recipe) * preference_start_cooking
        # shaped_reward = jnp.where((player_idx == 1), shaped_reward_1, shaped_reward)
        # shaped_reward = jnp.where((player_idx == 1), 0, shaped_reward)

        # shaped_reward = shaped_reward.astype(jnp.float32)

        # shaped_reward = 0.
        # shaped_reward_blue = (num_onions_in_pot < num_onions_in_recipe) * add_onion_to_pot * 10

        # shaped_reward = jnp.where((player_idx == 0), 0, shaped_reward)
        # shaped_reward = jnp.where((player_idx == 1), 0, shaped_reward)

        # shaped_reward = jnp.where((player_idx == 0), shaped_reward_0, shaped_reward)
        # shaped_reward = jnp.where((player_idx == 1), shaped_reward_1, shaped_reward)

        # reward = jnp.where((player_idx == 0), reward*1, reward)
        # reward = jnp.where((player_idx == 1), reward*1, reward)

        # reward = 0.
        # reward = jnp.where((player_idx == 0), shaped_reward_0*1, reward)
        # reward = jnp.where((player_idx == 1), shaped_reward_1*1, reward)

        shaped_reward = 0.
        shaped_reward = jnp.where((player_idx == 0), shaped_reward_0*1, shaped_reward)
        shaped_reward = jnp.where((player_idx == 1), shaped_reward_1*1, shaped_reward)


        take_item_from_counter = is_table * ~table_is_empty * inv_is_empty * object_is_pickable

        pick_up_dish = successful_pickup * (object_on_table == OBJECT_TO_INDEX["plate_pile"]) * inv_is_empty

        put_item_on_counter = successful_drop

        no_effect = jnp.logical_and(jnp.logical_and(~successful_pickup, ~successful_drop), ~successful_delivery)

        potting_onion = case_1
        potting_tomato = case_2

        high_level_dict = {'pickup_onion': add_onion_to_pot, 
                           'pickup_tomato': add_tomato_to_pot,
                           'pickup_dish': has_picked_up_plate,
                           'pickup_soup': pick_up_soup,
                           'soup_delivery': successful_delivery,
                           'potting_onion': potting_onion,
                           'potting_tomato': potting_tomato,
                           }


        # I do not want player 1 to do anything

        # reward = 0.

        #!!!!!!!!!!!!!!!!!! - just to test the sample !!!!
        # reward = reward_any_dish
        #!!!!!!!!!!!!!!!!!! - just to test the sample !!!!

        reward = shaped_reward * self.main_rew_is_shaped_rew + reward * (1.0 - self.main_rew_is_shaped_rew)

        #### 
        # shaped_reward = 0.
        # reward = 0.
        # reward_1 = (successful_delivery).astype(jnp.float32)
        # reward = jnp.where((player_idx == 1), reward_1, reward)
        # reward = jnp.array(successful_delivery, dtype=float)
        ...
        return maze_map, inventory, reward, shaped_reward, high_level_dict
