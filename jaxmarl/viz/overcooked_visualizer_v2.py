...
#######################################
    ...
    def render_single(self, state, agent_view_size=5):
        self._lazy_init_window()

        padding = agent_view_size - 2
        grid = np.asarray(state.maze_map[padding:-padding, padding:-padding, :])

        # Render grid into an image
        ...
        # Add textual information (e.g., score)
...
#######################################
    ...
    def _render_state(self, agent_view_size, state, highlight=True, tile_size=TILE_PIXELS):
        ...
        timestep_text = f"Step: {state.timestep}"
        draw.text((10, y_position), timestep_text, font=font, fill=(255, 255, 255))
