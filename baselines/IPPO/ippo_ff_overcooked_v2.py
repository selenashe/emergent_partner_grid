...
def get_rollout(train_state, config, model1_params=None, model2_params=None):
    ...
    network = ActorCritic(env.action_space().n + 1, activation=config["ACTIVATION"])
...
def make_train(config):
    ...
    def train(rng, model1_params=None, model2_params=None):
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                # SELECT ACTION
                # rng, _rng = jax.random.split(rng)
                # rng, _rng, rng_wait, rng_random_a1 = jax.random.split(rng, 4)
            ...
            # CALCULATE ADVANTAGE
            # train_state, env_state, last_obs, update_step, rng = runner_state
            train_state, env_state, last_obs, last_done, update_step, rng, model_status_1, wait_buffer_0, wait_buffer_1, slow_delay_0, slow_delay_1 = (
                runner_state
            )
...
def main(config):
    ...
    out = jax.vmap(lambda rng: train_jit(rng, model1_params=model1_params, model2_params=model2_params))(rngs)
