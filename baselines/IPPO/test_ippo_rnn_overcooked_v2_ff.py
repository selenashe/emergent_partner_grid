...
def get_rollout(train_state, config, rnn_params=None, model1_params=None, model2_params=None, 
                AGENT_1_1_SPEED=1, AGENT_1_2_SPEED=1, 
                seed_num=0, layout_name=None):
    
    env = jaxmarl.make(config["ENV_NAME"], **config["ENV_KWARGS"])
    network = ActorCritic_ff(env.action_space().n + 1, activation=config["ACTIVATION"])
    ...
    AGENT_0_COOLDOWN = 2
    carry_init = (key, state, obs, last_done, model_status_1, 0, 0)

    def step_fn(carry, _):
        key, st, obs, last_done_flag, mode, a0_cnt, a1_cnt = carry
        key, k0, k1, ks = jax.random.split(key, 4)

        obs = {k: v.flatten() for k, v in obs.items()}
        pi0, _ = network.apply(network_params, obs["agent_0"])
        pi1, _ = network.apply(network_params, obs["agent_1"])

        a0_raw = pi0.sample(seed=k0)
        ...
        return (key, st, obs, jnp.asarray([dn["__all__"]]), mode, a0_cnt_new, a1_cnt_new), (st, rw_val, mode, partner_tuple)


    T = int(config["ENV_KWARGS"]["max_steps"])
    (state_seq, reward_seq, agent1_modes, partner_tuple_seq) = lax.scan(step_fn, carry_init, jnp.arange(T))[1]
    ...
    artifact_basename = f"ff_rollout_data_seed_{seed_num}_layout_name_{layout_name}_speeds_{AGENT_1_1_SPEED}{AGENT_1_2_SPEED}"
...
def main(config):
    ...
    batch_filename = f"ff_rollouts_batch_{layout_name}_seeds_{seed_start}_to_{seed_end}.h5"
    ...
    with h5py.File(save_path, "a", libver="latest") as f_h5:
        for seed_num in range(seed_start,seed_end):
            for AGENT_1_1_SPEED, AGENT_1_2_SPEED in speed_pairs:
                ...
                state_seq, reward_seq, agent1_modes, agent1_speeds, rollout_data, rollout_key = get_rollout(train_state, config, loaded_ckpt_MLP, model1_params=model1_params, model2_params=model2_params,
