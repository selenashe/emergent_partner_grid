...
def make_train(config):
    ...
    def train(rng, preloaded_params=None, preloaded_params_2=None, model1_params=None, model2_params=None):
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                (
                    ...
                    slow_delay_0,
                    slow_delay_1,
                    scheduled_switch_timestep,
                ) = runner_state


                rng, rng_slow0, rng_slow1, rng_slow2, rng_mid1, rng_mid2, rng_midspd0, rng_midspd1, rng_midspd2, _rng = jax.random.split(rng, 10)
                ...
                #######
                raw_state   = env_state.env_state
                current_eps_time = raw_state.time
                schedule_switch = jax.random.uniform(rng_mid1, (config["NUM_ENVS"],)) < 0.5
                new_switch_time = jax.random.randint(rng_mid2, (config["NUM_ENVS"],), 180, 420)
                scheduled_switch_timestep = jnp.where(last_done_agent1, jnp.where(schedule_switch, new_switch_time, -1), scheduled_switch_timestep)

                switch_now = scheduled_switch_timestep == current_eps_time
                new_speed_0 = jax.random.choice(rng_midspd0, allowed_speeds, (config["NUM_ENVS"],))
                idx_low_mid = jax.random.randint(rng_midspd1, (config["NUM_ENVS"],), 0, 3)
                idx_high_mid = jax.random.randint(rng_midspd2, (config["NUM_ENVS"],), 0, 4)
                new_speed_1 = jnp.where(jnp.isin(new_slow_delay_0, low_set), high_set[idx_high_mid], low_set[idx_low_mid])

                slow_delay_0 = jnp.where(switch_now, new_speed_0, slow_delay_0)
                slow_delay_1 = jnp.where(switch_now, new_speed_1, slow_delay_1)
                new_delay_after_switch = jnp.where(model_status_1, slow_delay_1, slow_delay_0)
                wait_buffer_1 = jnp.where(switch_now, new_delay_after_switch, wait_buffer_1)
                #######
            ...
            initial_hstate = runner_state[-8]
            runner_state, traj_batch = jax.lax.scan(
                _env_step, runner_state, None, config["NUM_STEPS"]
            )

            # CALCULATE ADVANTAGE
            train_state, env_state, last_obs, last_done, update_step, hstate, rng, model_status_1, wait_buffer_0, wait_buffer_1, slow_delay_0, slow_delay_1, scheduled_switch_timestep = (
                runner_state
            )
        ...
        scheduled_switch_timestep_init = jnp.full(config["NUM_ENVS"], -1, dtype=jnp.int32)
        ...
        runner_state = (
            ...
            slow_delay_init_0,
            slow_delay_init_1,
            scheduled_switch_timestep_init,
        )
...
def main(config):
    ...
    wandb.init(
        ...
        name=f"switch_ippo_rnn_overcooked_v2_{layout_name}",
    )
