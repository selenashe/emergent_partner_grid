...
def get_rollout(train_state, config, network2_params=None, rnn_params=None, 
                ...
                AGENT_1_1_SPEED=1, AGENT_1_2_SPEED=1, 
                AGENT_2_1_SPEED=1, AGENT_2_2_SPEED=1, 
    ...
    AGENT_0_COOLDOWN = 2
    carry_init = (key, hstate_0, hstate_1, state, obs, last_done, model_status_1, 0, 0, 0)

    def step_fn(carry, t):
        key, h0, h1, st, obs, last_done_flag, mode, a0_cnt, a1_cnt, t = carry
        ...
        speed1 = jnp.where(t < 300, AGENT_1_1_SPEED, AGENT_2_1_SPEED)
        speed2 = jnp.where(t < 300, AGENT_1_2_SPEED, AGENT_2_2_SPEED)
        agent1_speed_target = jnp.where(mode == 0, speed1, speed2)
        ...
        partner_tuple = (speed1, speed2)
        return (key, h0, h1, st, obs, jnp.asarray([dn["__all__"]]), mode, a0_cnt_new, a1_cnt_new, t+1), (st, rw_val, mode, h0, partner_tuple)
    ...
    artifact_basename = f"switch_rollout_data_seed_{seed_num}_layout_name_{layout_name}_speeds_{AGENT_1_1_SPEED}{AGENT_1_2_SPEED}"
...
def main(config):
    ...
    wandb.init(
        ...
        # dir=wandb_dir,
        name=f"switch_{test_model}_{layout_name}",
    )
    ...
    # speed_pairs = [(6,0,0,6),(0,6,6,0)]

    speed_pairs_init = list(product([0,3],[5,6])) + list(product([8,9],[0]))

    results = []
    for i in range(len(speed_pairs_init)):
        for j in range(len(speed_pairs_init)):
            if i != j:
                results.append((*speed_pairs_init[i], *speed_pairs_init[j]))
    ...
    batch_filename = f"switch_rollouts_batch_{layout_name}_seeds_{seed_start}_to_{seed_end}.h5"
    save_path = f"/scratch/gpfs/rm4057/rollouts/{batch_filename}_{os.getpid()}.h5"
    save_path = f"rollouts/{batch_filename}_{os.getpid()}.h5"
    ...
    with h5py.File(save_path, "a", libver="latest") as f_h5:
        for seed_num in range(seed_start,seed_end):
            for AGENT_1_1_SPEED, AGENT_1_2_SPEED, AGENT_2_1_SPEED, AGENT_2_2_SPEED in speed_pairs:
                ...
                state_seq, reward_seq, agent1_modes, agent1_speeds, rollout_data, rollout_key = get_rollout(train_state, config, loaded_ckpt, loaded_ckpt_RNN, model1_params=model1_params, model2_params=model2_params,
                                                                                                            AGENT_1_1_SPEED=AGENT_1_1_SPEED, AGENT_1_2_SPEED=AGENT_1_2_SPEED, 
                                                                                                            AGENT_2_1_SPEED=AGENT_1_2_SPEED, AGENT_2_2_SPEED=AGENT_2_2_SPEED,
                                                                                                            seed_num=seed_num, layout_name=layout_name)
    ...
    # wandb.log({f"animation_{AGENT_1_1_SPEED}{AGENT_1_2_SPEED}": wandb.Video(f"{filename}.mp4", format="mp4")})
