...
def _binary_search_perplexity(
    distances, target, tol=1e-5, max_iter=200, sigma_min=1e-10, sigma_max=1e10
):
    ...
    def body_fun(val):
        ...
        sigma = jnp.where(
            perp > target, (sigma + sigma_min) / 2, (sigma + sigma_max) / 2
        )
...
def transform(
    ...
    *,
    ...
    X_new, _ = jax.lax.scan(
        train_step_early_exageration, X_new, xs=None, length=n_exageration
    )
